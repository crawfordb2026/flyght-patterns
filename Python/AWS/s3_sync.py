...
Usage:
  ...
  python s3_sync.py push --processed  # only data/processed/ CSVs
  python s3_sync.py push --results    # only analysis_results/
...
S3 layout (relative to bucket root, or prefix/ if configured):
  ...
  metadata.txt
  processed/dam_data_prepared.csv
  processed/ML_features_Z.csv
  analysis_results/pca/pca_scores.csv
  analysis_results/umap/umap_clusters.csv
  ...
"""
...
CATEGORIES = {
    ...
    "processed": {
        "local_dir": PYTHON_DIR / "src" / "csv_pipeline" / "data" / "processed",
        "s3_prefix": "processed",
        "exts": {".csv", ".txt"},
        "recursive": False,
    },
    "results": {
        "local_dir": (
            PYTHON_DIR / "src" / "sql_db_pipeline" / "analysis" / "analysis_results"
        ),
...
def parse_args():
    ...
    parser.add_argument("--processed", action="store_true", help="Include data/processed/")
    parser.add_argument("--results", action="store_true", help="Include analysis_results/")
...
def resolve_active(args):
    ...
    any_flag = args.raw or args.filtered or args.processed or args.results
    active_cats = {
        "raw": args.raw or not any_flag,
        "filtered": args.filtered or not any_flag,
        "processed": args.processed or not any_flag,
